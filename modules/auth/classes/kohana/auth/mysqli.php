...
abstract class Kohana_Auth_Mysqli extends Auth {
	...
	protected function _login($user, $password, $remember)
	{
        ...
        if ($failed_cnt >= $login_max_failed_attempts_from_ip) {
            ...
            IbHelpers::set_message('Unexpected system error (' . $error_id . ')');
		...
		if ($user['password'] === $password)
		{
            ...
            if (Settings::instance()->get('two_step_authorization') &&
                (($user['two_step_auth'] == 'SMS' &&
                        (!empty($sms_resource) && Model_Roles::has_permission($user['role_id'], $sms_resource['id'])))
                    || ($user['two_step_auth'] == 'Email' &&
                        (!empty($email_resource) && Model_Roles::has_permission($user['role_id'], $email_resource['id']))
                    )
                )
            ) {
				$this->two_step_auth = $user;
				$this->user = $user;
			} else {
				// Finish the login
	...
	protected function _step2_login($user_id, $code)
	{
        if (Model_Users::two_step_auth_check($user_id, $code)) {
            $user = Model_Users::get_user($user_id);
            // Finish the login
            $this->complete_login($user);

            // Log the login
            $this->login_log($user['email'], true);
            return true;
        } else {
            return false;
        }
	...
	public function force_login($user, $mark_session_as_forced = FALSE)
	{
		if ( ! is_object($user))
		{
			...
			$user = DB::Select()
					...
					->where('email', '=', $username)
					->execute();
			//I guess, this is bug in Kohana. Please check if I'm wrong.
			$user = $user->current();
		}
	...
	public function delete_expired_tokens()
	{
        ...
        protected function _login_with_external_provider($user, $remember)
    {
        // Complete the login
