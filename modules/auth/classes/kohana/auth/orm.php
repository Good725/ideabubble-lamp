...
abstract class Kohana_Auth_ORM extends Auth {
	...
	protected function _login($user, $password, $remember)
	{
		if ( ! is_object($user))
		{
			...
			$user->where($user->unique_key($username), '=', $username)->and_where('deleted', '!=', '1')->find();
		}
		...
		if ($user->has('roles', ORM::factory('role', array('name' => 'login'))) AND $user->password === $password)
		{
			if ($remember === TRUE)
			{
				...
				Cookie::set($GLOBALS['autologin'], $token->token, $this->_config['lifetime']);
			}
	...
	protected function login_log($user, $success=FALSE)
	{
		...
		$sql_log = ('INSERT INTO `engine_loginlogs` (`ip_address`, `email`, `time`, `success`, `user_agent`, `session`) VALUES (INET_ATON('.$db->escape(Request::$client_ip).'), \''.$user.'\', '.time().', '.$success.', \''.$user_agent.'\', '.$db->escape($session->id()).')');
		...
		if ($token = Cookie::get($GLOBALS['autologin']))
		{
			...
			if ($token->loaded() AND $token->user->loaded())
			{
				if ($token->user_agent === sha1(Request::$user_agent))
				{
					...
					// Set the new token
					Cookie::$httponly = true;
					Cookie::set($GLOBALS['autologin'], $token->token, $token->expires - time());
