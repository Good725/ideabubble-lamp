...
class Kohana_Database_MySQLI extends Database {
	...
	protected $_transactions_started = 0;
	...
	public function connect()
	{
		...
		unset($this->_config['connection']['username'], $this->_config['connection']['password']);
			...
			else
			{
				...
				$this->_connection = @new mysqli('p:' . $hostname, $username, $password);
			}
	...
	public function query($type, $sql, $as_object = FALSE, array $params = NULL)
	{
		...
		$use_multi_query = stripos(ltrim($sql), 'call ') === 0 ? true : false;
		
		// Execute the query
		if($use_multi_query){
			if ($this->_connection->multi_query($sql) === FALSE)
			{
				if (isset($benchmark))
				{
	...
	public function begin($mode = NULL)
	{
		...
		if ($this->_transactions_started == 0) {
            $result = (bool) $this->_connection->query('START TRANSACTION');
        } else {
            $result = true;
        }

        if ($result) {
            ++$this->_transactions_started;
        }
        return $result;
	}
	...
	public function commit()
	{
		...
		if ($this->_transactions_started == 1) {
            $result = (bool) $this->_connection->query('COMMIT');
        } else {
            $result = true;
        }
	...
	public function rollback()
	{
		...
		$result = (bool) $this->_connection->query('ROLLBACK');
        if ($result) {
            $this->_transactions_started = 0;
        }
        return $result;
	}
	...
	public function escape($value)
	{
		...
		if (($value = $this->_connection->escape_string( (string) $value)) === FALSE)
		{
			throw new Database_Exception(':error',
				array(':error' => $this->_connection->error),
				$this->_connection->errno);
