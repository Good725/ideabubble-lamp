...
/**
 * To add new language to project:
 * 1. add to project config file 'i18n.php' key 'allowed_languages' with array of language codes.
 *
 * To generate new translation files:
 * 1. wrap all strings into __()
 * 2. create directory projects/technopath/i18n - output will be stored here
 * 3. run: /usr/bin/php /Users/peter/Sites/wpp/projects/technopath/www/index.php --uri=i18n/generate --lang=ru
 *
 */
...
class Controller_I18n extends Controller {
	...
	public function action_generate()
	{
		...
		I18n::lang($options['lang']); // to retrive existing translations

		$files = Kohana::list_files(NULL, NULL); //array(APPPATH)
		...
		foreach ($files as $rel => $file)
		{
			...
			if ( strtoupper(pathinfo($file, PATHINFO_EXTENSION)) != 'PHP' ) continue;
			...
			foreach ($matches[1] as $key)
			{
				...
				$data[$key] = $class; // eliminating duplicates
			}
		}

		$content = View::factory('i18n/lang_file')->set('langs', $data)->set('skip_comments_in_beginning_of_included_view_file',true);
	...
	private function __write_file($lang, $content)
	{
		...
		$file = PROJECTPATH.'/' .$this->__lang_to_file($lang);
