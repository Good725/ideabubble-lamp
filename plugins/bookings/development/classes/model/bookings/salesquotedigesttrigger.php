...
class Model_Bookings_Salesquotedigesttrigger extends Model_Automations_Trigger
{
    const NAME = 'Sales Quote digest';
    ...
    protected function _join_delegate()
    {
        ...
        if ($joined == false) {
            $delegate_count_subquery = DB::select('booking_id', DB::expr("count(*) as delegatecount"))
                ->from(Model_KES_Bookings::DELEGATES_TABLE)
                ->group_by('booking_id');
            ...
            $this->filter_select->select_array(
                array(
                    ...
                    DB::expr('IF(students.id is null, lead_booker_mobiles.value, mobiles.value) as studentmobile'),*/
                ...
                ->join(array($delegate_count_subquery, 'delegate_count'), 'left')
    ...
    public function filter($data, $sequence)
    {
        ...
        $this->filter_select = DB::select(
            ...
            array('bookings.amount', 'bookingamount'),
            array('bookings.created_date', 'datecreated')
        )
            ->distinct('*')
        ...
        $tmpfile = tempnam(Kohana::$cache_dir, 'salesquotedigest');
        $fid = fopen($tmpfile, "c+");
        if (count($bookings) > 0) {
            ...
            return array(
                array(
                    ...
                    'automation_attachments' => array(
                        array(
                            'path' => $tmpfile,
                            'name' => 'salesquotedigest-' . date('Ymd') . '.csv'
                        )
                    )
                )
            );
