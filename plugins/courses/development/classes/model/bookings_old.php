...
class Model_Bookings extends Model
{
    ...
    public static function count_all($search = false)
    {
        ...
        $query = DB::query(Database::SELECT,
            "SELECT `plugin_courses_schedules`.`id` FROM `plugin_courses_bookings`
            LEFT JOIN
            `plugin_courses_schedules`
            ON
            `plugin_courses_schedules`.`id` = `plugin_courses_bookings`.`schedule_id`
            LEFT JOIN
			...
			WHERE
			`plugin_courses_bookings`.`paid` = 1 " . $_search . "
    ...
    {
        $query = DB::query(Database::SELECT,
            "SELECT first_name,last_name,address,email,gender,mobile,phone,teaching_co_reg,teaching_co_number,comments,school,school_address,roll_no,school_phone,school_county,county,date_modified FROM plugin_courses_bookings")
            ->execute()
            ->as_array();
        return $query;
    }
    ...
    public static function get_bookings($limit, $offset, $sort, $dir, $search = false)
    {
        ...
        if (count($query) > 0) {
            ...
            foreach ($query as $elem => $sub) {
                ...
                $query = DB::query(Database::SELECT, "SELECT `date_created` FROM `plugin_courses_bookings` WHERE `schedule_id` = " . $sub['id'] . " ORDER BY `date_created` DESC LIMIT 0,1")
                    ->execute()
                    ->as_array();
    ...
    public static function count_all_for_schedule($id, $search = false)
    {
        $_search = '';
        if ($search) {
            $_search = " AND (`plugin_courses_bookings`.`first_name` like '%" . $search . "%' OR `plugin_courses_bookings`.`last_name` like '%" . $search . "%' OR `plugin_courses_bookings`.`email` like '%" . $search . "%' OR `plugin_courses_bookings`.`phone` like '%" . $search . "%' OR `plugin_courses_bookings`.`comments` like '%" . $search . "%' OR `plugin_courses_bookings`.`school` like '%" . $search . "%')";
        }
        $query = DB::query(Database::SELECT,
    ...
    public static function get_all_for_schedule($id, $limit, $offset, $sort, $dir, $search = false)
    {
        $_search = '';
        if ($search) {
            $_search = " AND (`plugin_courses_bookings`.`first_name` like '%" . $search . "%' OR `plugin_courses_bookings`.`last_name` like '%" . $search . "%' OR `plugin_courses_bookings`.`email` like '%" . $search . "%' OR `plugin_courses_bookings`.`phone` like '%" . $search . "%' OR `plugin_courses_bookings`.`comments` like '%" . $search . "%' OR `plugin_courses_bookings`.`school` like '%" . $search . "%' OR `school_address` LIKE '%" . $search . "%' OR `roll_no` LIKE '%" . $search . "%' OR `school_phone` LIKE '%" . $search . "%' OR `plugin_courses_counties`.`name` LIKE '%" . $search . "%')";
        }
        $_limit = ' LIMIT ' . $offset . ',' . $limit;
        $query = DB::query(Database::SELECT,
            "SELECT `first_name`, `last_name`, `email`, `phone`, `comments`, `school`, `school_address`, `roll_no`, `school_phone`, `plugin_courses_counties`.`name` as `county`, `plugin_courses_bookings`.`paid`
                FROM
                `plugin_courses_bookings`
                LEFT JOIN
                `plugin_courses_counties`
                ON
                `plugin_courses_counties`.`id` = `plugin_courses_bookings`.`county_id`
                WHERE
                `plugin_courses_bookings`.`id` IS NOT NULL " . $_search . "
                AND
                `plugin_courses_bookings`.`schedule_id` =" . $id . $_search . " ORDER BY " . $sort . " " . $dir . " " . $_limit)
            ->execute()
            ->as_array();
        return $query;
    }
    ...
    public static function get_all_applicants($schedule)
    {
        if (empty($schedule)) {
            ...
            $query = DB::query(Database::SELECT, "SELECT first_name,last_name,address,email,gender,mobile,phone,teaching_co_reg,teaching_co_number,comments,school,school_address,roll_no,school_phone,school_county,county,date_modified FROM plugin_courses_bookings WHERE plugin_courses_bookings.schedule_id = " . $schedule)->execute()->as_array();
        }
        return $query;
    }
    ...
    public static function save_booking($data)
    {
        ...
        $query = DB::insert('plugin_courses_bookings', array_keys($insert))
            ->values($insert)
            ->execute();
        return $query;
    }
    ...
    {
        $query = DB::query(Database::SELECT, "SELECT * FROM `plugin_courses_bookings` WHERE
		...
		elseif (is_array($query) AND count($query) == 1)
		{
            ...
            DB::update('plugin_courses_bookings')
                ->set($data)
                ->where('id', '=', $query['0']['id'])
                ->execute();
            return;
	...
	{
		return DB::update('plugin_courses_bookings')->set($data)->where('id', '=', $id)->execute();
	}

	public static function get_booking_data($id)
	{
		$q =  DB::select()->from('plugin_courses_bookings')->where('id', '=', $id)->execute();
    ...
    public static function cart_paid($code, $amount, $channel, $address1 = NULL, $address2 = NULL)
    {
        if (isset($code) AND strlen($code) > 0) {
            ...
            $query = DB::query(Database::SELECT, "SELECT * FROM `plugin_courses_bookings`" . $in)
                ->execute()
                ->as_array();
            if (is_array($query) AND count($query) > 0) {
                ...
                DB::query(Database::UPDATE, "UPDATE `plugin_courses_bookings` SET
                `paid` = 1,
    ...
    {
        $query = DB::query(Database::SELECT, "SELECT `plugin_courses_bookings`.`id`, `plugin_courses_courses`.`summary`, `plugin_courses_schedules`.`start_date`
        FROM `plugin_courses_bookings`
        LEFT JOIN
        `plugin_courses_schedules`
        ON
        `plugin_courses_bookings`.`schedule_id` = `plugin_courses_schedules`.`id`
        LEFT JOIN
        `plugin_courses_courses`
        ON
        `plugin_courses_schedules`.`course_id` = `plugin_courses_courses`.`id`
        WHERE
        `plugin_courses_bookings`.`key` = '" . ($code) . "'")
            ->execute()
            ->as_array();
    ...
    public static function get_csv_report($id, $search = false)
    {
        ...
        `plugin_courses_counties`.`name` as `county`
                ...
                `paid` = 1 " . $_search . "
                AND
                `plugin_courses_bookings`.`schedule_id` =" . $id . $_search)
            ->execute()
            ->as_array();
        $return = array();
        $i = 0;
