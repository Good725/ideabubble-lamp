    ...
    ?>
		<?php foreach ($discounts AS $key=>$discount): ?>
			<tr data-discount_id="<?=$discount['id'];?>">
				<td><a href='<?php echo URL::Site('admin/courses/edit_discount/'.$discount['id']);?>'><?=$discount['id']?></a></td>
				<td><a href='<?php echo URL::Site('admin/courses/edit_discount/'.$discount['id']);?>'><?=$discount['title']?></a></td>
				<td><a href='<?php echo URL::Site('admin/courses/edit_discount/'.$discount['id']);?>'><?=$discount['summary'];?></a></td>
				<td><a href='<?php echo URL::Site('admin/courses/edit_discount/'.$discount['id']);?>'><?
                    ...
                    if ($discount['categories'] != '') {
                        $detail[] = 'courses in ' . Model_Coursediscounts::get_course_cats_from_discount_x($discount['categories']);
                    }
				...
				?></a></td>
				<td><a href='<?php echo URL::Site('admin/courses/edit_discount/'.$discount['id']);?>'><?=date('d-m-Y',strtotime($discount['valid_from']));?></a></td>
				<td><a href='<?php echo URL::Site('admin/courses/edit_discount/'.$discount['id']);?>'><?=date('d-m-Y',strtotime($discount['valid_to']));?></a></td>
				<td><a href='<?php echo URL::Site('admin/courses/edit_discount/'.$discount['id']);?>'>Edit</a></td>
				<td>
					<label>
