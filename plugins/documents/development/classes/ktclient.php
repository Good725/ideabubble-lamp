...
class KTClient {
    ...
    const MIGRATION_COMPLETE_FILE = '.kt_migrated';
    ...
    private function executeRequest($request, $parameters = array()) {
        ...
        if ( ! $this->kt_migrated)
        {
            ...
            for ($i = 0, $ok = FALSE; ! $ok AND $i < self::MAX_RETRIES; $i++)
            {
                try
                {
                    if ($request != 'login' AND $request != 'locate_folder_by_path')
                    {
                        Log::instance()->add(Log::INFO, '[KT_REQUEST] '.$request);
                    }

                    $response = $this->client->__soapCall($request, $parameters);
                    $ok       = $this->requestSuccessful($response);
                }
                catch (Exception $e)
                {
                    sleep(1);
            ...
            if (!$ok)
            {
                Log::instance()->add(Log::ERROR, get_class().': unable to complete the request.');
            }
            else
            {
                return $response;
        ...
        else
        {
            ...
            Log::instance()->add(Log::ERROR, '[KT_MIGRATION][EXECUTE_REQUEST]');
        }
    ...
    public function addFolder($name, $folder = 1) {
        ...
        try
        {
            if (is_int($folder))
            {
                $content = $this->browse($folder, 1, 'F');
                $folder  = $content[0]['full_path'];
            }

            $this->_create_path($folder.'/'.$name);

            if ( ! $this->kt_migrated AND Model_Files::get_directory_id($folder.'/'.$name) != $response->id)
            {
                Log::instance()->add(Log::ERROR, '[KT_MIGRATION][DIRECTORY_ID_MISMATCH]');
            }
        }
        catch (Exception $exception)
        {
            Log::instance()->add(Log::ERROR, '[KT_MIGRATION][EXCEPTION] '.$exception->getMessage().' ~ '.$exception->getFile().' [ '.$exception->getLine(). ' ]');
        }
        /** KT MIGRATION **/

        return $this->kt_migrated ? 0 : $response->id;
    }
    ...
    private function _create_path($path)
    {
        ...
        {
            if (Model_Files::get_directory_id($path.$directory) === NULL)
            {
                $parent_id = Model_Files::get_directory_id($path);
                $pk_id     = $this->kt_migrated ? NULL : $this->locateFolderByPath($path.$directory);

                Model_Files::create_directory($parent_id, $directory, $pk_id);
            }
    ...
    public function addDocument($filename, $localFilepath, $folder = 1, $title = null, $documentType = 'Default') {
        ...
        {
            $title or $title = $filename;
            $folderId = is_int($folder) ? $folder : Model_Files::get_directory_id('/'.$folder);
            $response = new stdClass();
        }
        ...
        try
        {
            if (is_int($folder))
            {
                $content = $this->browse($folder, 1, 'D');
                $folder  = $content[0]['full_path'];
            }

            $this->_create_path ($folder);
			$folderId = Model_Files::get_directory_id('/'.$folder);
            $this->_copy_file   ($folderId, $title, $localFilepath, $filename, $this->kt_migrated ? NULL : intval($response->document_id));
        }
        catch (Exception $exception)
        {
            Log::instance()->add(Log::ERROR, '[KT_MIGRATION][EXCEPTION] '.$exception->getMessage().' ~ '.$exception->getFile().' [ '.$exception->getLine(). ' ]');
        ...
        return $this->kt_migrated ? 0 : $response->document_id;
    }
     ...
     * @param int $pk_id
     */
    private function _copy_file($parent_id, $title, $local_file, $file_name, $pk_id)
    {
        $file_info = array
        (
            'name'     => $file_name,
        ...
        $this->lastFileId = Model_Files::create_file($parent_id, $title, $file_info, $pk_id);
    }
    ...
    public function browse($baseFolder = 1, $depth = -1, $type = 'DFS') {
        ...
        if ($this->kt_migrated)
        {
            return $this->_browse($baseFolder, $depth, $type);
        }
        /** KT MIGRATION **/
        ...
        $content    = $this->convertToArray($response->items);
        ...
        if ($depth < 0)
        {
            ...
            {
                if ($item['item_type'] == 'F' AND is_array($item['items']))
                {
                    $run_patch = FALSE;
            ...
            {
                // Log
                Log::instance()->add(Log::INFO, get_class().': browse returned no subfolders, trying to browse in safe mode.');

                $content = $this->browse_safe_mode($folderId, $type);
            }
        }
     ...
     * @return array|null
     */
    private function _browse($baseFolder, $depth = -1, $type = 'DFS', $level = 0)
    {
        $items = NULL;

        if ( ($directory_id = Model_Files::get_directory_id('/'.$baseFolder)) !== NULL)
        {
            $content = Model_Files::list_directory($directory_id);
            ...
            foreach ($content as $file)
            {
                ...
                (
                    'id'            => $file['id'           ],
                    'item_type'     => $file['type'         ] == Model_Files::FILE_TYPE_REGULAR ? 'D' : 'F',
                    'title'         => $file['name'         ],
                    'filename'      => $file['name'         ],
                    'filesize'      => $file['size'         ],
                    'created_date'  => $file['date_created' ],
                    'modified_date' => $file['date_modified'],
                    'full_path'     => preg_replace('/\/+/', '/', $baseFolder.'/'.$file['name']),
                    'mime_display'  => NULL,
                    'items'         => NULL
                );

                if ($file['type'] == Model_Files::FILE_TYPE_DIRECTORY AND ($depth < 0 OR $level < $depth))
                {
                    $item['items'] = $this->_browse($baseFolder.'/'.$file['name'], $depth, $type, $level + 1);
                }
    ...
    private function browse_safe_mode($folderId, $type)
    {
        $parameters = array($folderId, 1, $type);
        $response   = $this->executeRequest('get_folder_contents', $parameters);
        ...
        if (is_array($content))
        {
            ...
            {
                if ($content[$i]['item_type'] == 'F')
                {
                    $content[$i]['items'] = $this->browse_safe_mode($content[$i]['id'], $type);
                }
            }
        }
    ...
    public function downloadDocument($documentId, $version = null) {
        ...
        /** KT MIGRATION **/
        if (Model_Files::file_exists($documentId))
        {
            try
            {
                Model_Files::download_file($documentId);
        ...
        {
            Log::instance()->add(Log::INFO, '[KT_MIGRATION][FILE_NOT_FOUND]' . $documentId);
        }
        /** KT MIGRATION **/
    ...
    public function downloadDocumentToFolder($documentId, $path, $version = null) {
        ...
        if (Model_Files::file_exists($documentId))
        {
            ...
            try
            {
                Model_Files::get_file($documentId, $path, $time);
            }
            catch (Exception $exception)
            {
                Log::instance()->add(Log::ERROR, '[KT_MIGRATION] '.$exception->getMessage().' ~ '.$exception->getFile().' [ '.$exception->getLine(). ' ]');
            }
        ...
        if ( ! $this->kt_migrated)
        {
            ...
            $file = file_get_contents($response->message);

            $time = time();
            $template = $path.$time.'.docx';
            $template_file = $time.'.docx';

            $open = fopen($template, "w");
            if ($open)
            {
                fwrite($open, $file);
                fclose($open);
     ...
     * Custom Function
     *
     * Should move document from one location to another
     *
     * @param $documentId - needed
     * @param $baseFolder - path to customer directory
     * @param $reason - a message as to why you are moving the file
     * @return mixed - xml service response
     * @author david - the cool dude that wrote the function
     *
     */
    public function moveDocument($documentId, $baseFolder, $reason) {

        if ( ! $this->kt_migrated)
        {
            // Get the folder ID based on the path to the folder e.g. /contacts/15122/agent_documents/
            $folderId = $this->locateFolderByPath($baseFolder);

            // Create array of parameters for move document request
            $parameters = array($documentId, $folderId, $reason);
            // Execute the move request
            $response = $this->executeRequest('move_document', $parameters);
        }
        else
        {
            $folderId = is_int($baseFolder) ? $baseFolder : Model_Files::get_directory_id('/'.$baseFolder);
            $response = new stdClass();
        }
        ...
        try
        {
            $this->_create_path($baseFolder);

            Model_Files::move_file($documentId, $folderId);
        }
        catch (Exception $exception)
        {
            Log::instance()->add(Log::ERROR, '[KT_MIGRATION][EXCEPTION] '.$exception->getMessage().' ~ '.$exception->getFile().' [ '.$exception->getLine(). ' ]');
