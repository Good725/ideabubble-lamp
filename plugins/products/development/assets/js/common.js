    ...
    $('body')
        .on('click', '.edit', function() {
            ...
            var c = DYNAMIC_TABLE.get_configuration($(this).parents('table').attr('id')),
                o = window[$(this).data('objects_array')][$(this).data('object_id')],
                sort_by = $('.sorting_asc').index(),
                order = 'asc',
                rows_per_page = $('select[name=products_table_length]').val(),
                page = $('.dataTables_paginate li.active a').text();;

            if(sort_by == -1){
                sort_by = $('.sorting_desc').index();
                order = 'desc';
            }

            window.location = c.edit_url + '?id=' + o.id + '&rows_per_page=' + rows_per_page + '&sort_by=' + sort_by + '&sort_order=' + order+ '&page=' + page;
        })
...
jQuery(function(){
    jQuery('#datetimepicker_from').datetimepicker({
        ...
        onShow:function( ct ){
            this.setOptions({
                maxDate:jQuery('#datetimepicker_to').val()?jQuery('#datetimepicker_to').val():false
            })
        }
    });
    jQuery('#datetimepicker_to').datetimepicker({
        ...
        onShow:function( ct ){
            this.setOptions({
                minDate:jQuery('#datetimepicker_from').val()?jQuery('#datetimepicker_from').val():false
            })
        }
    });
});
    ...
    for (var i = 0; i < c.columns.length; i++) {
        ...
        switch (c.columns[i]) {
            ...
            case 'publish':
                ...
                span_value = ((data.publish == '1') ? '<i class="icon-ok"><span style="display: none;">1</span></i>' : '<i class="icon-remove"><span style="display: none;">0</span></i>');
                break;
