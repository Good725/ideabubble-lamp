...
final class Model_DiscountRate extends Model
{
    // Tables
    const MAIN_TABLE = 'plugin_products_discount_rate';
	 ...
	 * @param boolean $for_role only show discount rates, applicable to the logged-in user
     * @return array If the function succeeded, it returns TRUE. Otherwise, it returns FALSE.
     */
    public static function get($id = NULL, $where_clauses = NULL, $for_role = FALSE, $in_date_range = FALSE, $categories = array())
    {
        $q = DB::select('t1.id', array('t2.id', 'format_id'), array('t2.title', 'title'), array('t2.type_id', 'type_id'), 'range_from', 'range_to', 'discount_rate', 'discount_rate_percentage', 't1.publish', 't1.date_modified', 't1.date_entered', 't1.modified_by', 't1.created_by')
			->from(array(self::MAIN_TABLE, 't1'))
			    ->join(array(Model_DiscountFormat::MAIN_TABLE, 't2'), 'LEFT')
			        ->on('t1.format_id', '=', 't2.id')
                ->join(array('plugin_products_discount_format_has_categories', 'has_categories'), 'left')->on('t2.id', '=', 'has_categories.discountformat_id')
            ->where('t1.deleted', '=', 0)
        ...
        if (is_array(@$categories) && count($categories) > 0) {
            $q->and_where('has_categories.category_id', 'in', $categories);
        } else {
            if ($categories === null) {
                $q->and_where('has_categories.category_id', 'is', null);
            }
        }
        ...
        if ($where_clauses != NULL)
        {
            for ($i = 0; $i < count($where_clauses); $i++)
            {
                ...
                if ( ! in_array($clause[0], array('format_id', 'title', 'type_id')) AND strpos($clause[0], '.') === FALSE)
                {
		...
		// Only show discounts applicable to the logged-in user
		if ($for_role)
		{
			$user = Auth::instance()->get_user();

			if (Settings::instance()->get('user_discount_overwrite') == 1)
			{
				// If a discount is specified at user level, apply that discount
				$q
					->join('engine_users', 'RIGHT')
					->on('engine_users.discount_format_id', '=', 't2.id')
					->where('engine_users.id', '=', $user['id']);
			}
			else
			{
				// Otherwise apply discounts that would normally be applicable to that user
				$q
					->join(array('plugin_products_discount_format_users_roles', 'format_role'),'LEFT')
					->on('format_role.discountformat_id', '=', 't2.id')
					->and_where_open()
						->where('format_role.usersrole_id', 'is', NULL)
						->or_where('usersrole_id', '=', $user['role_id'])
					->and_where_close();
			}
		}
		...
		if ($in_date_range)
		{
			$q->where(DB::expr("'".date('Y-m-d H:i:s')."'"), 'BETWEEN', array(DB::expr('`t2`.`date_available_from`'), DB::expr('`t2`.`date_available_till`')));
		}
        ...
        for ($i = 0; $i < count($r); $i++)
        {
            $r[$i]['type'] = ($r[$i]['type_id'] == NULL) ? NULL : Model_DiscountFormat::$types_description[$r[$i]['type_id']];
    ...
    private function set_common_values(&$array)
    {
        ...
        $array['range_from'   ] = $this->main_table['range_from'   ];
        $array['range_to'     ] = $this->main_table['range_to'     ];
        $array['discount_rate'] = $this->main_table['discount_rate'];
        $array['publish'      ] = $this->main_table['publish'      ];
        $array['discount_rate_percentage'] = $this->main_table['discount_rate_percentage'];
    }
    ...
    {
        $r = DB::select('id', 'format_id', 'range_from', 'range_to', 'discount_rate', 'publish', 'deleted', 'date_modified', 'date_entered', 'modified_by', 'created_by')
                 ->from(self::MAIN_TABLE)
                 ->where('id', '=', $id)
                 ->execute()
                 ->as_array();
