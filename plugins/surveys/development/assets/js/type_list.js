$(document).ready(function ()
{
    ...
    $(document).on("click", ".publish", function (ev)
    {
        ...
        $.post('/admin/surveys/ajax_publish_type', {id: id, state: state}, function (data)
        {
            if (data.status === 'success')
            {
                if (state === 1)
                {
                    ...
                    $("#main").prepend('<div class="alert alert-success"><a class="close" data-dismiss="alert">&times;</a><strong>Success: </strong> Type successfully Archived.</div>');
                }
                else
                {
                    $(".publish[data-id='" + id + "']").html('<i class="icon-ok"></i>').data('publish', 1);
                    $("#main").prepend('<div class="alert alert-success"><a class="close" data-dismiss="alert">&times;</a><strong>Success: </strong> Type successfully published.</div>');
                }
            }
            else
            {
    ...
    $("#btn_delete_yes").click(function (ev)
    {
        ev.preventDefault();
        var id = $(this).data('id');
        $.post('/admin/surveys/ajax_delete_type', {id: id}, function (data)
        {
            if (data.status === 'success')
            {
                initTable('#subjects_table');
                $("#main").prepend('<div class="alert alert-success"><a class="close" data-dismiss="alert">&times;</a><strong>Success: </strong> Type successfully removed.</div>');
            }
            else
            {
                $("#main").prepend('<div class="alert alert-error"><a class="close" data-dismiss="alert">&times;</a><strong>Error: </strong> ' + data.error_msg + '</div>');
    ...
    {
        var ajax_source = "/admin/surveys/ajax_get_all_types";
        var settings = {
