...
class Model_Todos_TaskAssignedTrigger extends Model_Automations_Trigger
{
    const NAME = 'a Task is assigned';
    ...
    public function filter($data, $sequence)
    {
        ...
        $this->filter_select = DB::select(
            ...
            'todos.priority',
            'todos.summary',
            DB::expr("DATE_FORMAT(todos.datetime, '%d %M %Y') as examstartdate"),
            DB::expr("DATE_FORMAT(todos.datetime_end, '%d %M %Y') as examenddate"),
            DB::expr("DATE_FORMAT(todos.datetime, '%H:%i') as examsstarttime"),
            DB::expr("DATE_FORMAT(todos.datetime_end, '%H:%i') as examendtime"),
            array('todos.grading_type', 'gradingtype'),
            DB::expr("DATE_FORMAT(todos.datetime_end, '%d/%m/%Y %H:%i') as duedate"),
        ...
        if (
            ...
            if (
                ...
                ->join(array(Model_Contacts3::CONTACT_NOTIFICATION_RELATION_TABLE, 'trainer_mobiles'), 'left')
                    ...
                    ->on('trainer_mobiles.deleted', '=', DB::expr(0))
                ->join(array(Model_Contacts3::CONTACT_NOTIFICATION_RELATION_TABLE, 'trainer_emails'), 'left')
                    ...
                    ->on('trainer_emails.deleted', '=', DB::expr(0))
            ...
            switch ($condition['field']) {
                case 'todo_status':
                    if (
                        ...
                        call_user_func(array($this->filter_select, $condition_function), 'assignees.status', $operator, $condition['val']);
                    } else {
                ...
                case 'end_date_interval':
                    ...
                    break;
                case 'end_date':
                    call_user_func(array($this->filter_select, $condition_function), DB::expr('CAST(todos.datetime_end as DATE)'), $operator, $condition['val']);
                    break;
                default:
