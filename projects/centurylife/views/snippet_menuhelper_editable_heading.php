<ul class="<?=$class_name;?>" >
    ...
    <li id="item_<?=$menulv0['id']?>" class="level_<?=$menulv0['level']?> item_<?=$menulv0['item_number']?> <?=$menulv0['first']?> <?=$menulv0['last']?> <?=$menulv0['current']?>"><a target="<?=$menulv0['menus_target']?>" href="<?=$menulv0['link']?>"><h1><?=$menulv0['title']?></h1></a>
    <?php if($menulv0['has_sub'] == '1'):?>
        <?php $lv1 = menuhelper::submenu($menulv0); ?>
        <ul class="level2">
            <?php foreach($lv1 as $menulv1): ?>
            <li id="item_<?=$menulv1['id']?>" class="level_<?=$menulv1['level']?> item_<?=$menulv1['item_number']?> <?=$menulv1['first']?> <?=$menulv1['last']?> <?=$menulv1['current']?>"><a target="<?=$menulv1['menus_target']?>" href="<?=$menulv1['link']?>"><?=$menulv1['title']?></a></li>
