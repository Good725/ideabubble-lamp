		...
		<div class="form-group">
			...
			<div class="form-control">
				<div class="input_group">
					...
					<input type="text" name="payment_total" id="payment_total" class="form-input validate[required,custom[onlyNumber],min[<?=Settings::instance()->get('payonlineform_min') ?: 25?>]]" value="<?=(isset($_GET['amount'])) ? $_GET['amount'] : '';?>" placeholder="Numbers only" />
				</div>
			</div>
		</div>
				...
				<div class="form-controls">
					...
					<div class="select">
						<select name="location" id="location" class="form-input validate[required]">
							...
							<?php foreach (Model_Locations::get_parent_locations() as $plocation) { ?>
							<option value="<?=$plocation['name']?>"><?=$plocation['name']?></option>
							<?php } ?>
						</select>
					</div>
				</div>
        ...
        <div id="credit_card_payment">
			...
			<div class="form-group">
				...
				<div class="form-controls payment_form-ccExp">
					<div class="select">
						<select name="ccExpMM" id="ccExpMM" class="form-input validate[required,funcCall[checkCCDates]] cc-data">
							<?php
