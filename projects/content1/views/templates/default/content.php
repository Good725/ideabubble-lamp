        ...
        <div id="wrapper">
            ...
            <div id="content">
                <div id="content_area" class="content_area <?=(Settings::instance()->get('content_location') == 'right') ? 'right' : 'left'?>">
                    ...
                    <div id="content_panels_wrapper" class="left">
                        <?= Model_Panels::get_panels_feed('content_content'); ?>
                    </div>
                    <div id="user_content" class="user_content <?= $content_location ?>">
                        <?php
                        ...
                        switch ($page_data['name_tag']) {
                            ...
                            case 'contact-us.html':
                                $sites = array("saoirsetreatmentcenter");
                                if(in_array(Kohana::$config->load('config')->get('db_id'),$sites))
                                {
                                  // form added via CMS
                                }
                                elseif(Kohana::$config->load('config')->get('db_id') == 'lsomusic')
                                {
                                    echo View::factory('templates/default/template_views/lso_contact_us');
                                }
                                else
                                {
					...
					<?php if (isset($show_news_feed) AND $show_news_feed == TRUE): ?>
						<?php
						$news        = Model_News::get_plugin_items_front_end_feed('News');
						$show_news   = ((strpos($news, '<ul') != FALSE));
						?>
						<?php if ($show_news): ?>
							<div class="side_news_feed">
								<?= $news ?>
							</div>
						<?php endif; ?>
					<?php endif; ?>
