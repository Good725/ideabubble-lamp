...
<div id="checkout_cart"<?= (Settings::instance()->get('cart_hidden_when_empty') == 1) ? ' class="minicart-hidden-when-empty"' : ''; ?> data-product_count="<?= $number_of_items ?>">
    <div class="mycart">
        <div class="mycart_summary">
            <div class="mycart_summary_txt"><a href="/checkout.html" id="cart-btn">Shopping Cart</a></div><div class="amount" id="cart-item-num"><span class="mycart_items_amount"><?= $number_of_items ?></span></div>
            <?php if(isset($cart->lines) AND count($cart->lines) > 0): // don't render when there is no products ?>
                <div class="mycart_summary_details mycart_details">
                    <?php foreach($cart->lines as $line_number =>$line): ?>
						...
						{
							$thumbnail = HTML::entities(Model_Media::get_path_to_media_item_admin(Kohana::$config->load('config')->project_media_folder,$product['images'][0], Model_Product::MEDIA_IMAGES_FOLDER.DIRECTORY_SEPARATOR.'_thumbs'));
						}
						elseif (isset($product['thumb_url']) AND $product['thumb_url'] != '')
						{
							$thumbnail = $product['thumb_url'];
						}
						else
						{
							$thumbnail = HTML::entities(Model_Media::get_path_to_media_item_admin(Kohana::$config->load('config')->project_media_folder,'no_image_available.jpg', Model_Product::MEDIA_IMAGES_FOLDER.DIRECTORY_SEPARATOR.'_thumbs'));
						}
                        ?>

                    <div class="mycart_summary_details_line product_line" data-product_id="<?=$line->product->id?>" data-line_id="<?=$line_number?>">
                        <div class="mycart_summary_img"><img class="mini_cart_thumbnail" alt="<?= $line->product->title ?>" src="<?= $thumbnail ?>" /></div>
                        <div class="mycart_summary_itename" title="<?=$line->product->title ?>"><?=$line->product->title ?><span class="mycart_summary_itename_amount"> (<span class="quantity"><?=$line->quantity ?></span>)</span></div>
                        <div class="mycart_summary_price"><span class="price_total_line">€<?=$line->price ?></span></div>
                    </div>
                    <?php endforeach;?>
                    ...
                    <div class="mycart_summary_details_line">
                        ...
                        <div class="mycart_summary_price">€<span id="mycart_total_price"><?=Model_Checkout::get_cart_total_price_value()?></span></div>
                    </div>
                    <a href="/checkout.html">
                        <div class="button button-primary pl_button" id="pl_member_registration_button" name="pl_member_registration_button" type="button">Checkout</div>
                    </a>
                </div>
            <?php endif; ?>
        </div>
...
<link href="/assets/<?= $assets_folder_path ?>/css/minicart.css" rel="stylesheet" type="text/css"/>