<form action="<?=Request::initial()->uri()?>" method="get" class="paging-bl">
	...
	$end = $start + 5;
	$previous = ($current_page - 1 > 0 ? $current_page - 2 : null);
	$next = ($current_page + 1 < $end ? $current_page + 1 : null);
	?>

	<?php if($previous): ?>
		<button class="paging paging-prev" href="<?= $uri.'?'.http_build_query( array( 'page' => $previous ) + $pagination_parameters ) ?>">previous</button>
	...
	<?php if($next): ?>
		<span class="separator">|</span>
		<button class="paging paging-next" href="'<?= $uri.'?'.http_build_query( array( 'page' => $next ) + $pagination_parameters ) ?>">next</button>
	<?php endif; ?>

	<?php if ($pages > 0): ?>
		<p>Showing <?= $current_page * $per_page + 1 ?> &ndash; <?= ($current_page + 1) * $per_page ?> of <?= $pages ?> entries</p>
	<?php endif; ?>
</form>
