...
checkout_data.display_empty_cart = function(){
    ...
    $('#checkoutTable .totalprice, #checkoutTable .subtotal').html(0);
    $('.cart_hidden').hide();
...
checkout_data.add_to_cart_callback = function(status, data){
    switch(status){
        case CHECKOUT.STATUS_S_OK:
            ...
            location.href = '/frontend/radarstores/redirect_to_checkout?last_product=' + location.href;
            break;
    ...
    CHECKOUT.setPostalZone(zone_id, function(status,data){
        switch(status){
            case CHECKOUT.STATUS_S_OK:
                ...
                break;
            case CHECKOUT.STATUS_E_ERROR:
                display_message('error', 'Sorry, we have experienced an issue checking out this product. Please contact us today for help purchasing online - support@militaryandcamping.com');
                valid_cart = false;
                break;
            default:
                display_message('error', 'Sorry, we have experienced an issue checking out this product. Please contact us today for help purchasing online - support@militaryandcamping.com');
