...
\n    IF(`lead_booker_mobiles`.`country_dial_code` IS NOT NULL AND `lead_booker_mobiles`.`country_dial_code` != \'\' , CONCAT_WS( \'\', \'+\', `lead_booker_mobiles`.`country_dial_code`, `lead_booker_mobiles`.`dial_code`, `lead_booker_mobiles`.`value`), `lead_booker_mobiles`.`value`) AS `Lead booker mobile`,
