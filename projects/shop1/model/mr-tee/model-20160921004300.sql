/*
ts:2016-09-21 00:43:00
*/

DELIMITER ;;

UPDATE IGNORE `plugin_pages_layouts` SET
  `source` = '<?php $page_data[''product_data''] = Model_Product::get_by_product_url(''oh-crop''); ?>
...
<head>
	...
	<?php if ( ! empty($page_data[''product_data''][''images''][1])): ?>
		<?php $base_path = str_replace(''.dev'', ''.ie'', Model_Media::get_path_to_media_item_admin(Kohana::$config->load(''config'')->project_media_folder,'''', ''products'')); ?>
	...
	<?php Kohana::find_file(''template_views'', ''custom_background'')?>
...
<div id="wrap">
	<div id="container">
		...
		<div id="footer">
			<?php Kohana::find_file(''template_views'', ''footer'') ?>
		</div>
	</div>
</div>
