...
$(document).on('change','#builder_quantity',function(){
    $.post('/frontend/products/check_quantity_price',{quantity:$("#builder_quantity").val(),base_price:$("#static_price").text()},function(data){
        data = $.parseJSON(data);
        $(".offer_price").show();
        $(".offer_percent_discount").text(data.percent_off);
        $('#unit_price').text(data.unit_price);
        $(".full_price").text(data.total_amount);
    });
});