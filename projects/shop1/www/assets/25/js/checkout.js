...
checkout_data.add_to_cart_callback = function(status, data){
    switch(status){
        ...
        case CHECKOUT.STATUS_E_MISSING_OPTIONS:
            display_message('error', 'Please fill out the missing options.');
            valid_cart = false;
            break;
...
function submitCheckout(){
    ...
    if(subbmit_status){
        ...
        $.ajax({
            ...
            .fail(function(data){
                display_message('error', 'Error: Network error, please check your internet conexion');
            });
    }
    else{
...
function updateProductOptionPrice(item,product_id){
    //Initialize price variables
    var current_price = parseFloat($('#final_price').data('product_price'));
    var options_price = 0;

    $.post('/frontend/products/check_stock_levels',{option_id:$("#"+item.getAttribute('id')).val(),product_id: product_id},function(result){
        var obj = jQuery.parseJSON(result);
        if(obj.is_stock_item == true)
        {
            if(parseInt(obj.quantity) > 0)
            {
                $("#"+item.getAttribute('id')).find(':selected').data('option_price',parseFloat(obj.price));
        ...
        $('.prod_option_item').each(function(){
