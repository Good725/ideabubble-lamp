    ...
    if(status == CHECKOUT.STATUS_S_OK){
        ...
        $.post('/frontend/frontend/get_template/mini_cart', function(data, status){
            if(status == 'success'){
				...
				$cart.css('height', $cart.height).html(data);
				setTimeout(function()
				{
					$('.show-minicart-on-add').css('z-index', 1).slideDown();
				}, 1);
            }
        });
    }
...
function updateProductOptionPrice(obj,product_id)
{
    ...
    var total = (current_price + options_price) * $final_price.data('conversion');
    total = total.toFixed(2);
    $final_price.html($final_price.data('currency') + total);
...
$(document).on('mouseout', '.mycart_summary_details', function()
{
	this.style.display = '';
});