...
class Kohana_Form {
			...
			foreach ($options as $value => $name)
			{
				if (is_array($name))
				{
					...
					foreach ($name as $_value => $_name)
					{
						...
						if (in_array($_value, $selected) || $selected === ['{all}'])
						{
				...
				else
				{
					...
					if (in_array($value, $selected) || $selected === ['{all}'])
					{
						// This option is selected
						$option['selected'] = 'selected';
					}
	...
	public static function ib_captcha($captcha_version = 2)
    {
        require_once ENGINEPATH.'/plugins/formprocessor/development/classes/model/recaptchalib.php';
        return recaptcha_get_html(Settings::instance()->get('captcha_public_key'), null, $captcha_version);
    }
